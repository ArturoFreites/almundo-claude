    ...
    cards: [
      {
        ...
        descriptionHtml: 'SQLite with WAL mode for concurrent writes. Stores decisions, commits, iterations and events in <code>.claude/almundo-memory.db</code> inside each project. 0600 permissions by default.',
      },
      {
        title: 'Smart search',
    ...
    faq: [
      {
        ...
        answerHtml: 'In the file <code>.claude/almundo-memory.db</code> inside each project root. It\'s a local SQLite file; nothing is sent to external services. Add it to <code>.gitignore</code> if you don\'t want to version it.',
      },
      {
      ...
      },
      {
        ...
        answerHtml: 'Yes. Simply delete the <code>.claude/almundo-memory.db</code> file. You can also deactivate memory from <strong>/alfred-dev:config</strong>: existing data is preserved but stops being queried and no new events are captured.',
      },
    ],
  },
...
---`,
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        answerHtml: 'It\'s a local SQLite database that stores each project\'s decisions, commits and iterations. Optionally activated from <strong>/alfred-dev:config</strong>. Once active, Almundo IA automatically records workflow events and the <strong>Librarian</strong> agent can answer historical queries like "why was this architecture chosen" or "what was done in the last iteration", always citing sources. Data stays within the project: everything is kept in <code>.claude/almundo-memory.db</code>.',
      },
      {
  ...
  changelog: [
    ...
    },
    {
      ...
      date: '2026-03-13',
      added: [
        'SQLite to native memory sync layer: decisions, iterations, and commits stored in almundo-memory.db are automatically projected as .md files in ~/.claude/projects/<hash>/memory/ using native Claude Code format.',
