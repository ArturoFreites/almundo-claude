#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Hook de SessionStart para el plugin Almundo IA (fork interno basado en alfred-dev).
#
...
# Describe quién es Almundo IA y qué puede hacer (sobre la base técnica de alfred-dev).
CONTEXT="## Almundo IA - tu empresa de ingeniería en un plugin (basado en alfred-dev)

Tienes a tu disposición un equipo completo de agentes especializados:
Orquestador Almundo (coordinación), El Buscador de Problemas (producto), El Dibujante de Cajas (arquitectura),
...
- /almundo-ia:feature <descripción> - Nuevo desarrollo con flujo completo (producto -> arquitectura -> desarrollo -> calidad -> docs -> entrega)
- /almundo-ia:fix <descripción> - Corregir un bug (diagnóstico -> corrección -> validación)
- /almundo-ia:spike <descripción> - Investigación exploratoria (exploración -> conclusiones)
- /almundo-ia:ship - Preparar release (auditoría -> docs -> empaquetado -> despliegue)
- /almundo-ia:audit - Auditoría completa del código (calidad + seguridad + simplificación)
- /almundo-ia:config - Ver o modificar la configuración del plugin
- /almundo-ia:status - Estado de la sesión de trabajo activa
- /almundo-ia:update - Comprobar y aplicar actualizaciones del plugin
- /almundo-ia:help - Ayuda detallada de todos los comandos
...
if [[ -f "$CONFIG_FILE" ]]; then
  if ! CONFIG_CONTENT=$(cat "$CONFIG_FILE"); then
    echo "[Almundo IA] Aviso: no se pudo leer '$CONFIG_FILE'" >&2
    CONFIG_CONTENT=""
  fi
...
El usuario ha definido preferencias en .claude/alfred-dev.local.md (nombre heredado del upstream alfred-dev):
...
try:
    ...
    print(f'[Almundo IA] Aviso: estado de sesión corrupto o incompleto: {e}', file=sys.stderr)
    sys.exit(0)
...
Puedes continuar la sesión con /almundo-ia:status o avanzar a la siguiente fase."
  fi
fi
...
MEMORY_DB="${PROJECT_DIR}/.claude/almundo-memory.db"
...
db.close()
" "$MEMORY_DB" "$PLUGIN_ROOT" 2>/dev/null || echo "[Almundo IA] Aviso: no se pudo crear la BD de memoria" >&2
fi
...
# Configuración local de Almundo IA (nomenclatura heredada de alfred-dev)

Este fichero se genera automáticamente en la primera sesión.
Puedes personalizarlo con `/almundo-ia:config`.
LOCALCFG
else
...
# Si el proyecto tiene memoria (.claude/almundo-memory.db), se extrae
...
if command -v curl &>/dev/null; then
  LATEST_RELEASE=$(curl -s --max-time 3 --proto '=https' \
    -H "User-Agent: almundo-claude-plugin" \
    "https://api.github.com/repos/almundo/almundo-claude/releases/latest" \
    | python3 -c "
import json, sys
try:
...
Hay una nueva versión de Almundo IA (fork basado en Alfred Dev): v${LATEST_RELEASE} (actual: v${CURRENT_VERSION}). Ejecuta /almundo-ia:update para actualizar."
  fi
fi
