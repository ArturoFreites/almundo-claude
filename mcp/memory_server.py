#!/usr/bin/env python3
"""
Servidor MCP stdio para la memoria persistente de Almundo IA.
...
Al arrancar, el servidor resuelve la ruta de la DB relativa al directorio
    de trabajo (``$PWD/.claude/almundo-memory.db``), abre la conexion, asegura
...
logging.basicConfig(
    stream=sys.stderr,
    level=logging.DEBUG,
    format="[almundo-memory] %(asctime)s %(levelname)s: %(message)s",
    datefmt="%H:%M:%S",
)
_log = logging.getLogger("almundo-memory")
...
class MemoryMCPServer:
    ...
    def _handle_initialize(
        ...
        return _make_response(
            request_id,
            {
                ...
                "serverInfo": {
                    "name": "almundo-memory",
                    "version": "0.4.2-almundo-claude.0",
                },
                "capabilities": {
                    "tools": {},
                },
    ...
    def run(self) -> None:
        ...
        """
        _log.info("Servidor MCP almundo-memory iniciado")
        ...
        _log.info("Servidor MCP almundo-memory finalizado")
...
def main() -> None:
    ...
    retention_str = os.environ.get("ALMUNDO_MEMORY_RETENTION_DAYS", "365")
    try:
