---
description: "Comprueba y aplica actualizaciones del plugin Almundo IA"
---

# Actualizar Almundo IA

Eres Alfred. El usuario quiere comprobar si hay una version nueva del plugin. Sigue estos pasos al pie de la letra.

## Paso 1: obtener la versión instalada
...
candidates = sorted(
    glob.glob(os.path.expanduser('~/.claude/plugins/cache/almundo-claude/**/.claude-plugin/plugin.json'), recursive=True),
...
Si no se puede leer, busca la version en el fichero `plugin.json` mas cercano dentro de `~/.claude/plugins/cache/almundo-claude/`. El script selecciona la version mas reciente por fecha de modificacion para evitar errores cuando coexistan varias versiones en cache.
...
```bash
curl -s --max-time 10 "https://api.github.com/repos/almundo/almundo-claude/releases/latest"
```
